...
L:RegisterTranslations("enUS", function() return {
  ...
  knock_away_trigger1 = "s Knock Away hits",
  ...
  bw_bar = "Possible Blast Wave",
  bw_bar_first = "First Blast Wave",
  knock_away_possible_bar = "Possible Knock Back",
...
L:RegisterTranslations("deDE", function() return {
  ...
  bw_bar = "Mögliche Druckwelle",
  bw_bar_first = "Druckwelle",
  knock_away_possible_bar = "Möglicher Knock Back",
...
function BigWigsBroodlord:Event(msg)
  ...
  elseif string.find(msg, L["bw_trigger"]) and self.db.profile.bw then
    if GetTime() - self.lastbw > 5 then
      self:TriggerEvent("BigWigs_StopBar", self, L["bw_bar"])
      self:TriggerEvent("BigWigs_StartBar", self, L["bw_bar"], 20, "Interface\\Icons\\Spell_Holy_Excorcism_02", true, "Red")
      self:ScheduleEvent("BigWigs_Message", 15, L["bw_warn"], "Urgent", true, "Alert")
      self:SetCandyBarFade("BigWigsBar "..L["bw_bar"], 15)
    end
    self.lastbw = GetTime()
  elseif string.find(msg, L["knock_away_trigger1"]) then
    self:TriggerEvent("BigWigs_StopBar", self, L["knock_away_possible_bar"])
    self:TriggerEvent("BigWigs_StartBar", self, L["knock_away_possible_bar"], 12, "Interface\\Icons\\INV_Gauntlets_05", true, "Red")
    self:ScheduleEvent("BigWigs_Message", 7, L["knock_away_possible_bar"], "Urgent", true, "Alert")
    self:SetCandyBarFade("BigWigsBar "..L["knock_away_possible_bar"], 13)
  end
end
...
function BigWigsBroodlord:CHAT_MSG_MONSTER_YELL(msg)
  ...
  if string.find(msg, L["engage_trigger"]) and not self.started then
    self:TriggerEvent("BigWigs_StartBar", self, L["bw_bar_first"], 20, "Interface\\Icons\\Spell_Holy_Excorcism_02", true, "Red")
    self:ScheduleEvent("BigWigs_Message", 15, L["bw_warn"], "Urgent", true, "Alert")
    self:SendEngageSync()
    self:StartFight()

    self:TriggerEvent("BigWigs_StartBar", self, L["knock_away_possible_bar"], 20, "Interface\\Icons\\INV_Gauntlets_05", true, "Red")
    self:ScheduleEvent("BigWigs_Message", 15, L["knock_away_possible_bar"], "Urgent", true, "Alert")
    self:SetCandyBarFade("BigWigsBar "..L["knock_away_possible_bar"], 5)
  end
end
